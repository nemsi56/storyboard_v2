...
Braid view (ported from `../Timeline`'s `updates_v1` branch, added as a "Strip /
Braid" toggle nested inside Timeline view rather than a 5th top-level view mode),
and a chron-strip lane-row/offscreen polish pass; not merged to `main`, explicitly
scoped to stay off `main` and every other branch until it's ready.)*
