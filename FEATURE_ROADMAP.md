...
| `thruLine_v2` | Forked from `thruLine_v1`, **not merged anywhere** — Chronology Ordinal mode now renders two or more scenes on different storylines that share an identical anchor (date + time, when set) at the same vertical, instead of always staggering them; scoped to Ordinal only (True scale's per-lane collision-avoidance pass makes the same idea materially harder/riskier there, discussed but not attempted). Grouping is adjacency-scoped in `chronOrder` and never merges two scenes on the *same* storyline. A new showcase sample project (`longfellow-job.json`, a heist story built to exercise every conflict type plus the simultaneous-anchor alignment) surfaced a second round: the Conflicts panel now scrolls to/centers a conflict's scenes on both the Chronology and Narrative rows, and selecting a card vs. clicking a conflict row are now mutually exclusive (used to both stay highlighted at once); a stray section-color border on Narrative cards removed (was also silently breaking the selection-ring CSS); the Narrative row's first section now actually shows its name; clipped anchor date/time inputs in the Inspector fixed; Mac keyboard shortcut labels (⌘/⌥) added (the underlying handler already accepted Cmd, this was display-only); and a Path-view flashback investigation that turned up no bug, just a sample restructured into a proper in-medias-res cold open to actually demonstrate the feature. Most recently: the Inspector/Conflicts panel background now matches its own active tab (was a shade off from the New/Edit Scene pane's equivalent), and the "Conflicts (N)" header freezes at the top while the list scrolls beneath it. Also discussed but explicitly deferred (not implemented): scoping offscreen scenes to the Chronology rows only, excluded from Cards/Flow/Narrative and their numbering — assessed as a real but non-trivial change (five separate places independently rebuild "scenes in display order" and would all need updating, including Reports, which the user hadn't mentioned) — see `STATUS.md` for the full writeup. Most recently: a fifth full-app audit found and fixed six real issues (a Timeline "New Scene" discard-guard bypass, missing `nextId`/`nextSecId` repair + section validation in the v3 import path, a project-creation write-ordering bug that could leave a phantom project entry on storage failure, a chart section-filter/lettering bug, an unnecessary O(n²) conflict recompute on every save, and a zoom-slider save-per-tick perf issue), plus a same-session follow-up round of smaller efficiency fixes (a redundant per-lane scene-number rebuild in the charts, layout-thrashing in the section-header alignment code, and a stale build-script file list). Most recently: a ten-item direct-feedback round — Reveals/Foreshadow UI polish (context-correct empty-state copy, the "Requires knowing" → "Foreshadow" rename, edit/delete buttons on reveal library entries, and the New Scene form gaining the full Timing + Reveals field set Edit Scene already had) plus five real bugs found and fixed (a Loom wire staying highlighted after deselecting its card; a board-drag bug that snapped horizontal scroll back to the left edge and made dragging toward an off-screen section impossible, now also with edge auto-scroll; a stray section-header artifact left behind when switching Cards to Timeline; an intermittent-failure Timeline drag caused by a debounced background re-render destroying the drag's own ghost element; and a storyline/section drag that could leave the Inspector spuriously "dirty" with Save wrongly greyed out). Most recently: the Reveal/Foreshadow display labels were swapped (display-only — data fields and conflict-engine logic unchanged) after user correction; a Cancel-deselects-the-card bug in the reveal-edit modal fixed; reveal edit/delete modal titles made context-aware; New Scene's Cancel/Save now grey out when blank; and a multi-round Path view (Braid) polish pass — section-divider labels added (including a previously-missing first one) and frozen against vertical scroll, the "READING ORDER"/"CHRONOLOGY" axis labels converted from scrolling SVG text to a `position:sticky` HTML overlay frozen in the top-left corner in both scroll directions, both the axis labels and section headers now sitting on one continuous opaque margin bar (sized to the viewport, hiding scrolled-past grid content) instead of letting it show through, a stacking-order bug in that same change caught and fixed (an explicit z-index on a sibling was silently painting over the section labels), the selected node in Path view made visibly more prominent (thicker ring, slight scale-up, glow) without recoloring it away from its own storyline color, and the confusing "Reveal used before shown" conflict message reworded to match the new Reveal/Foreshadow terminology. Also fielded (and confirmed correct via live reproduction, not a bug) a question about why a specific Reveal/Foreshadow conflict wasn't appearing. See `STATUS.md` for the full narrative. |
| `thruLine_v3` | Forked from `thruLine_v2`, **not merged anywhere** — a Chronology lane-column overhaul: storyline rows now fill down to use available vertical space instead of stopping at content height (later reverted to content-sized growth after it overcorrected — see below), a real native scrollbar for storylines that don't fit under the height cap (previously silently clipped and unreachable) with the "+ Storyline" button moved to a true flex-sibling footer so it stays reachable regardless of scroll position, and the cap raised 55% → 65%. Three bugs surfaced and fixed along the way: trackpad horizontal-scroll jitter from an overzealous wheel-forwarding listener, wires bleeding through the lane label column (a pre-existing gap only newly visible once more of the column became reachable), and Path view's vertical "CHRONOLOGY" axis label being clipped with a mis-centered arrow (a `flex-direction` + `writing-mode:vertical-rl` interaction). Reconsidered mid-session: the fill-down fix was reverted back to content-sized growth (`flex:0 0 auto`) since always filling to the 65% cap pushed the button/caption down by a large fixed amount even for sparse projects — the cap now acts purely as a ceiling for many-storyline projects, with a fixed 24px margin giving the button its breathing room instead. Separately, fixed the zoom-to-fit setting to actually fit: three independent hardcoded floors (the fit calculation's own, a matching one in three `cardW` formulas, and a subtler one from `box-sizing:border-box` padding/border alone imposing a ~16px minimum) were silently overriding "fit everything, no scroll" once a project had enough scenes, verified fixed with a real 89-scene test case on both Loom and Path. A follow-up investigation into why Narrative looked more condensed than Chronology at that same zoom level found no bug — Chronology spreads scenes across N lanes while Narrative shows them in one continuous row, so identical card sizes read very differently — addressed instead with a new hover callout: every chron/manuscript card now shows a full-info popover (title, date, storyline, summary, POV, characters, locations) on a 400ms hover delay, independent of the card's rendered size. Also fixed: the "Create your first scene" empty-state message straddling into the New Scene pane (a missing `position:relative` on its containing block), and a real reveal/foreshadow conflict-engine bug — an item foreshadowed on more than one scene could have its payoff dragged back past a *later* foreshadow scene with no conflict firing, since the check only tracked "foreshadowed at least once so far," satisfied permanently by the earliest hint. Root-caused and verified against the reporting user's actual project file. See `STATUS.md` for the full narrative. |
| `thruLine_v4` | Forked from `thruLine_v3`, **not merged anywhere** — a Narrative/Chronology toggle above the Path view chart. Narrative mode (default) is unchanged from before. Chronology mode walks `S.chronOrder` across the columns (merging scenes sharing an exact anchor into one " / "-joined node), with rows equal to each column's own position — not a second independent axis — so the chart is always a monotonic southeast staircase, matching Loom's own Chronology row (a mid-round correction after the first pass mirrored reading order onto the rows instead, which the user caught as the wrong model). Both modes are draggable to reorder — Chronology-mode drags reorder `S.chronOrder`, Narrative-mode drags reorder reading order — through the same move-confirmation flow Loom's own chron/manuscript drags already use, so Path and Loom now stay in sync in both directions with no new wiring since both read/write the same underlying state. Also fixed a latent drag-cleanup bug (surfaced by, but not caused by, this round) where ending a drag could clear the wrong same-id DOM element's fade state. Follow-up direct-feedback rounds: the mode-switch button order flipped (Chronology, then Narrative), the top axis relabeled "Events →", a large low-opacity mode watermark added, and a real drag-reliability bug fixed (a forced-layout reflow on every mousemove during a drag, which could make a real move silently fail to register); era markers fixed to render in Chronology mode too (a mode-aware row lookup, not the axis limitation it first looked like); a leading era marker's clipping fixed twice (the first attempt just traded one collision for another); and a merged-node color collision with a storyline's own color fixed (a dedicated desaturated gray, guaranteed distinct from the fully-saturated storyline palette, replacing `var(--acc)`). Most recently: a full sample-project lineup overhaul — Pride and Prejudice retired (single-storyline, fully linear, so it demonstrated none of the Timeline features being showcased), Count of Monte Cristo converted from v2 to v3-native with 5 real storylines and a deliberately non-linear chronology matching its actual parallel revenge plots, and a new from-scratch Dracula sample (4 storylines, real 1893 diary dates, genuine simultaneous cross-storyline scenes) replacing it — with `ensureSampleProjects()` extended to retire a sample outright (deleting an untouched copy, preserving and unflagging an edited one) using the same `sampleKey`/`revision`-0 machinery the existing duplicate-prevention fixes already established. Most recently: documented (comments only, no behavior change) the reveals/requires field-vs-label swap that tripped up that same sample-authoring work, adding the explanation to `state.js` and `conflicts.js` — the two other places someone editing this data is likely to be looking — rather than renaming the fields, which was assessed and rejected as a breaking change to every saved/exported project for a purely cosmetic fix. Most recently: the Tutorial's first-ever Timeline documentation (storylines, anchors, Loom, Path, Conflicts, reveals/foreshadowing — every field/button label cross-checked against the real UI rather than paraphrased) plus a hypertext table of contents, and a matching "Timeline: Loom & Path" row added to the landing page (a new accent color needed — the existing 6 were all already spoken for). Most recently: a multi-round Tutorial redesign — the TOC reworked into five grouped "part" cards (thumbnail + one-sentence purpose + topic links, replacing the earlier per-item description list), content reorganized under matching visual "Part" banners for Getting Started/Cards/Flow/Timeline/Reference, and screenshots moved out of the TOC into the actual content next to each feature they illustrate with a new click-to-enlarge modal (`tutorial-init.js`) — plus a real user-supplied app icon (`images/icon.png`) rolled out to replace the 📝 emoji across the landing page, Tutorial, Projects, and the editor header, with the icon+wordmark on Projects/editor now linking back to the landing page. See `STATUS.md` for the full narrative. |
| `thruLine_v5` | Forked from `thruLine_v4`, **not merged anywhere** — a PayPal donate button added to the top-right header of the landing page and Projects page (dropped into each page's existing right-aligned header slot, no new layout), with both pages' CSP widened (`form-action`, `img-src`) to allow the PayPal submission/assets it needs. Landing page closing section: "Urge to donate?" removed from the contact line, replaced with a dedicated "If you like SceneSetter, please consider a donation to continue its development." line above "Enjoy!". Also fixed the landing header's mobile breakpoint, which previously hid the now-populated header slot outright. Most recently: offscreen scenes reworked into a strictly Chronology-only concept — no display number and excluded entirely from the Scene Board and Scene Flow Chart (previously shown on both despite docs claiming otherwise), with a new "+ New Scene" button in the Timeline Inspector's empty state (reparenting Card view's own New Scene form, replacing the old create-blank-then-edit flow) as the way to create one now that they're hidden from Cards/Flow; the Inspector's Edit/New forms also now show the full field set Card view's New Scene tab always had (Themes/Misc/Word Count/POV/Notes were previously hidden there). Two real bugs found via user reports: the Inspector could get stuck showing both its empty state and a leftover New Scene form at once, and the Narrative/Chronology toggle silently reset on every Cards↔Path round-trip without the buttons saying so (now persists instead, per a follow-up request). Also added storyline drag-reorder to Loom view (a ⠿ handle, mirroring the same pattern already used for Sections/Library items). Most recently: the Cards/Flow/Timeline view switch, previously reparented between each view's own toolbar, now lives permanently in a new centered `#menu-center` on the menu-bar row (flanked by flex:1 spacers, same trick `#hdr` uses for the project name); Timeline's own Ordinal/True-scale/Chronology-Narrative/Thread/Zoom selectors reparent up next to it while Timeline is open. The Timeline strip header's separate "Conflicts (N)" badge button was removed, its count folded into the Conflicts tab's own label (red when unselected and non-zero). A polish pass followed: the view-switch label's contrast fixed (theme `--tx` at reduced opacity instead of a separately-muted color), a thin light border added to its buttons on dark themes, the buttons/icons shrunk slightly, the Path-view icon's connecting line thinned for a "beads on a string" look, and Flow view gained an empty-state library-highlight hint for when Trace is off (mirroring the existing trace-category hint). Most recently: a new "Chronology" report type added next to "Scene List" in the Generate Report modal (same Include Fields checkboxes, same shared section filter), listing scenes in Path view's own Chronology-axis order via the existing `braidChronColumns()` — noting offscreen scenes and merging simultaneous ones the same way that view does. Most recently: help mode (`?` button) gained full Timeline coverage — nine new zones for Loom/Path controls it previously had zero tooltips for, plus two stale Cards/Flow tips fixed; a dozen-plus direct Tutorial corrections (each checked against the real DOM/JS/CSS before publishing, catching two factual errors in the user's own dictated replacement text along the way); every keyboard-shortcut reference removed from both the Tutorial and the app's own menus except Undo/Redo, per a direct report that the rest don't reliably work (display-only — the underlying Alt-key handlers in `editor.js` were left as-is); and the landing page's "Character & Element Library" and "Sections & Organization" features merged into one "Library & Sections" entry at #2, using the same two-screenshot side-by-side layout Scene Flow Chart/Timeline already use, with everything after renumbered down by one. See `STATUS.md` for the full narrative. |
| `thruLine_v6` | Forked from `thruLine_v5`, **not merged anywhere** — a full-app audit (code efficiency, bugs, security/data integrity) plus fixes, delegated across three parallel sub-audits (Timeline view, Editor/board, Charts/Reports/Conflicts/misc) with every finding re-verified against the actual code or reproduced live before being trusted. Security/data posture came back strong (CSP intact, zero XSS sinks, thorough v3 import validation) — no vulnerabilities found. Key discovery: `main`/`release` are still schema v2, so schema v3 (this branch's own work) has never shipped publicly, making it the last cheap moment to fix the long-standing `scene.reveals`/`scene.requires` field-vs-UI-label swap (§ earlier entries) — done: renamed to `scene.foreshadows`/`scene.payoffs` (matching their UI labels) across `state.js`/`editor.js`/`conflicts.js`/`projects.js` and all four sample projects, with a `sc.foreshadows ?? sc.reveals` load-time fallback for any pre-rename local data; DOM element ids left unrenamed (internal wiring only) to keep the diff contained. Bugs found and fixed: opening any project could silently record a phantom data-edit (`orderedUsedPovEntities()`'s POV-order self-heal was calling `recordDataEdit()`/`saveState()` as a render side effect — falsely dirtied the backup-status nag and permanently excluded any merely-opened sample from future `SAMPLES_VERSION` refreshes via the revision-0 check); Ctrl+Z could silently corrupt the wrong Library/POV entity if pressed while its Edit modal was open (undo/redo wasn't gated by `anyModalOpen()`, and those modals save back to an array index captured at open time); a window/panel resize during a Timeline chron/manuscript/Path drag could destroy the drag's own ghost element (the `ResizeObserver`'s debounced re-render was missing the `isTlDragActive()` guard `scheduleConflictsRecompute()` already had for the identical hazard); `pruneDismissed()` forced a second full synchronous conflict recompute on every single save once anything had been dismissed via "mark intentional" (now prunes against the existing cached conflict list instead, one-save lag, self-corrects); the search box rebuilt the entire board on every keystroke (now debounced 150ms); the bilocation conflict check's half-open interval test missed an instant scene sitting exactly at the *start* of another scene's duration window; v3 import didn't type-check `summary`/`notes` (a non-string could later crash search's `.toLowerCase()`); `duplicateProject()` and the import dialog's deferred "Update Local Copy"/"Keep Both" callbacks had no storage-failure guard (unlike every other project-creation path). Also: the reveal-order conflict check's O(N×R) per-payoff rescan replaced with a single precomputed lookup map, and four confirmed-dead code paths removed (`chronTrueScaleGapDivider`/`fmtGap` in timeline.js, `toggleFlagMode` in conflicts.js, an unused `scenesByStoryline` map rebuilt on every Timeline render). Every fix verified live in the browser (fresh preview-server port each time — this environment's dev-server setup was found to serve stale JS from Chrome's HTTP cache after edits, not a real app bug, but worth remembering), including a from-scratch bilocation repro, an instrumented-`renderBoard` keystroke-coalescing check, and a real Ctrl+Z-while-modal-open repro via actual keyboard events (not synthetic `dispatchEvent`, which doesn't reliably reach this app's listeners in headless testing). Most recently, a real user-reported bug: on Firefox specifically, cards near the end of a section could render clipped behind the next section, stuck regardless of scroll position, at certain zoom levels — root-caused (after the Chromium-only testing tool failed to reproduce it despite an extensive sweep, which itself was the tell) to Firefox's `flex-direction:column;flex-wrap:wrap` containers not reliably auto-sizing to fit every wrapped column the way Chromium/Safari do; `alignSecHeaders()` (editor.js) measured that already-too-narrow Firefox layout and permanently pinned it. Fixed by no longer trusting any browser's auto-sizing for the measurement pass — each section is first forced to a guaranteed-generous explicit width (worst case, one column per card), then measured, then shrunk to the real value needed. Verified as a true no-op on Chromium (identical final pixel widths before/after, across zoom/resize/detail-toggle); the reporting user then confirmed live in Firefox that the issue is resolved. Most recently: a second closing full-app audit (same delegated-and-independently-verified methodology) found one real regression in the first audit's own `pruneDismissed()` change — `undo()`/`redo()` saved through a stale conflicts cache after restoring a snapshot, which could silently un-dismiss a still-valid "mark intentional" conflict — fixed by refreshing the cache right after snapshot restore in both. Every other open finding from that first audit was also fixed this round: New Scene inline "+ Add" flows (character/POV/reveal minting) now correctly enable Save/Cancel instead of leaving them disabled until an unrelated field was touched; a Characters-library drag-reorder now refreshes both POV checklists like every other Characters mutator; deleting a POV-referenced character now preserves its dragged position in `povOrder` instead of losing it to the end; `updateAxisAvailability()`'s True-scale fallback now actually persists; `renderTimeline()` split into a shared tail plus a lean `renderTimelineZoomOnly()` path (skips the zoom-independent storyline-lane and thread-picker rebuilds) that the zoom slider now uses; chron/manuscript card drags now cache card positions once at drag-start instead of forcing a synchronous layout reflow on every mousemove (porting the fix Braid's own drag already had); `validateV3Import` now validates the legacy `reveals`/`requires` keys pre-rename files fall back to, not just the renamed ones; and `test-init.js`'s Editor/Projects-Page tests — dead code gated behind DOM elements that don't exist on test.html, so 7 of 24 tests silently never ran — now run for real, which surfaced and let fix a second bug the same investigation found: test.html was missing `timeline.js`/`conflicts.js` from its own script list, so editor.js's top-level `ESCAPE_ACTIONS` array threw a `ReferenceError` referencing timeline.js's `closeMarkerPopover` and silently halted the rest of editor.js's execution on that page. Every fix verified live (fresh preview-server port each time): a real undo/redo repro in both directions, a real slider-drag zoom check, chron/manuscript drag-to-reorder via direct mouse-event dispatch through to the move-confirmation modal, the New Scene mint-and-save flow, the POV-position-preserving delete, and test.html now passing 24/24 with a clean console (previously the `ReferenceError` fired silently even while the visible count read as passing). See `STATUS.md` for the full narrative. |
