...
const HELP_ZONES = [
  ...
  { sel: '#view-toggle',     tip: 'Cards / Flow / Timeline — switch between the card board, the Scene Flow Chart (Snake or Circle), and the Timeline (Loom or Path). Your last-used Flow layout and Timeline layout are each remembered next time you open one.' },
  ...
  { sel: '#chart-legend',    tip: 'Legend — appears when tracing items, sizing by word count, or as a reminder to select Library items to highlight matching scenes. Explains what each color or mark means; hover an entry to highlight its line on the chart.' },
  { sel: '#chart-print-btn', tip: 'Print — opens a clean, full-color printable version of the current chart in a new tab.' },
  { sel: '#tl-axis-switch',     tip: 'Ordinal / True Scale — Loom\'s horizontal axis. Ordinal spaces every scene evenly regardless of date; True Scale spaces them proportionally to the real time between them.' },
  { sel: '#tl-braid-mode-switch', tip: 'Chronology / Narrative — Path\'s axis. Narrative (the default) walks reading order; Chronology walks event order instead, merging scenes that share the exact same date/time into one node.' },
  { sel: '#tl-thread-wrap',     tip: 'Thread — pick a character to draw a highlighted line connecting every scene they appear in, tracing their journey through the Timeline.' },
  { sel: '#tl-zoom-ctl',        tip: 'Zoom — stretches or compresses scene spacing (Loom) or node spacing (Path). Double-click the slider to reset it to the midpoint.' },
  { sel: '#tl-lane-labels',     tip: 'Storylines — each row is a storyline lane. Drag the ⠿ handle to reorder lanes, or click + Storyline to add one.' },
  { sel: '#tl-chron-scroll-wrap', tip: 'Chronology strip — drag a scene card to change its date/time and storyline. Era markers and section boundaries appear along the strip; the wires below connect each card to its place in the reading order.' },
  { sel: '#tl-ms-scroll',       tip: 'Manuscript ribbon — the reading order row. Drag a card here to reorder scenes in the narrative.' },
  { sel: '#tl-braid-scroll',    tip: 'Path chart — one node per scene (or per group of simultaneous scenes, in Chronology mode), connected in reading order. A dashed segment marks a flashback — a scene that, chronologically, happened before the one just before it in reading order. Drag a node to move it.' },
  { sel: '.tl-panel-tabs',      tip: 'Inspector / Conflicts — Inspector shows the selected scene\'s fields, or a + New Scene button when nothing\'s selected. Conflicts lists every detected timeline problem (its count turns red here when any exist) — click a row to highlight its scenes, or mark it intentional to dismiss the warning.' },
